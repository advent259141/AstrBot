...
AstrBot 内置了**强大的** Agent 执行器，用户也可以选择接入第三方的 Agent 执行器服务，例如 Dify、Coze、阿里云百炼应用、DeerFlow 等，或者通过插件注册自定义的 Agent 执行器。
...
如果你是插件开发者，可以通过 `context.register_agent_runner()` 注册自定义的 Agent 执行器。详见 [注册自定义 Agent 执行器](/dev/star/guides/agent-runner)。
